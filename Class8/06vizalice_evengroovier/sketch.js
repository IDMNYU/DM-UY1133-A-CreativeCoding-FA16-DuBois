...
function draw() {
  ...
  for(i in termfrequency)
  {
    ...
    if(tfidf>=thresh)
    {
      ...
      termfrequency[i].x = 0.99*termfrequency[i].x + 0.01*termfrequency[i].tx;
      termfrequency[i].y = 0.99*termfrequency[i].y + 0.01*termfrequency[i].ty;
      if(abs(termfrequency[i].x-termfrequency[i].tx)<1) termfrequency[i].tx=random(width);
      if(abs(termfrequency[i].y-termfrequency[i].ty)<1) termfrequency[i].ty=random(height);
    }
  }
}
