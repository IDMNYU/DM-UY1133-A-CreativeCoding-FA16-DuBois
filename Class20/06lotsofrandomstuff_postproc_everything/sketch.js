...
function makeScene()
{
  ...
  // postprocessing
	...
	composer.addPass( new THREE.RenderPass( scene, camera ) );
 	...
 	e1 = new THREE.ShaderPass( THREE.DotScreenShader );
 	e1.uniforms[ 'scale' ].value = 4;
 	//effect.renderToScreen = true;
 	composer.addPass( e1 );

 	e2 = new THREE.ShaderPass( THREE.RGBShiftShader );
 	e2.uniforms[ 'amount' ].value = 0.2;
 	e2.renderToScreen = true;
 	composer.addPass( e2 );
...
function onDocumentMouseMove( event ) {
  ...
  var my = event.y / window.innerHeight;
 	e2.uniforms[ 'amount' ].value = mx;
 	e2.uniforms[ 'angle' ].value = (1.0-my)*.05;
}
